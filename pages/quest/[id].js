...
const Id = () => {
  ...
  useEffect(() => {
    const getFormApp = async (regionItemId) => {
      setLoadingDb(true);
      try {
        get(child(tasksRef, `regions/${regionItemId}/${id}`))
          .then((snapshot) => {
            if (snapshot.exists()) {
  ...
  const setQuizResult = (obj) => {
    if (obj) {
      ...
      update(dbRef, { completeQuizCount: itemQuest.completeQuizCount + 1 })
        .then(() => {
          setResult(obj);
        })
        .catch((err) => {
          ...
          >
            ...
            <div style={{ marginBottom: "20px" }}>
              <span style={{ marginBottom: "10px" }}>
                {result.numberOfCorrectAnswers} з {result.numberOfQuestions}{" "}
                запитань
              </span>
              <h3 style={{ color: "#fa520f" }}>
                <span>
            ...
            <div>
              ...
              <LinkedinShareButton url={"https://uaquiz.vercel.app"}>
